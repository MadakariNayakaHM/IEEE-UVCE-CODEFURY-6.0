  ...
  try {
    ...
    try {
      await sendEmail({
        ...
        message: ` from PsychTech Innovatica you have posted a request ,  ${sender.name} to the consulter ${reciver.name}\n please wait for some time until you get responded by the professional`,
      });
    } catch (err) {
    ...
    try {
      await sendEmail({
        ...
        message: `welcome mail from PsychTech Innovatica you have recived request ,  ${reciver.name} from the user ${sender.name}\n please reply  `,
      });
...
exports.updateAppointment=async (req,res,next)=>
{
    ...
    try {
        await sendEmail({
          ...
          message: `welcome mail from PsychTech Innovatica your post is been updated with description,  from ${data.to.name} , Thankyou `,
        });
      } catch (err) {
        console.log("error while sending email");
      }
      ...
      try {
        await sendEmail({
          ...
          message: `welcome mail from PsychTech Innovatica your post is been updated with description,  to ${data.to.name} , Thankyou `,
        });
      } catch (err) {
        console.log("error while sending email");
      }
...
exports.payAppointment=async(req,res,next)=>{
  const pid = req.body.pId;
  const post = await Post.findByIdAndUpdate(pid,{payment:req.body.payment},{ new: true, runValidators: true })
